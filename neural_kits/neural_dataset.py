...
class spatial_only_neural(Dataset):
    ...
    def append_new_data(self, new_firing_rates, new_labels):
        self.firing_rates = torch.cat([self.firing_rates, new_firing_rates], dim=0)
...
class vit_neural_temp_dataset(Dataset):
    def __init__(self, dataset, label):
        self.data = torch.FloatTensor(dataset)
        self.data_with_temp = self.data.clone()
        self.label = torch.LongTensor(label)
        ...
        self.label = self.label[:, None]
        self.label = repeat(self.label, 'b () -> b t', t=t)
        self.label_with_temp = self.label.clone()
...
class vit_nueral_multiresolution(Dataset):
    def __init__(self, dataset, label, base=8*10, divide=8):
        self.data = torch.FloatTensor(dataset)
        self.label = torch.LongTensor(label)

        _, t, _ = self.data.shape
        assert t == base
        assert base % divide == 0

        self.data = torch.chunk(self.data, chunks=divide, dim=1)  # chunk: number of chunks to return
        self.data = torch.cat(self.data, dim=0)

        self.label = torch.cat([self.label for i in range(divide)], dim=0)
