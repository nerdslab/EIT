...
class MAE_trans(MAE):
    """forward function is different, it takes two imgs at the same time"""
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.style = nn.Parameter(torch.randn(1, 16*16, 512))
    ...
    def forward(self, imgs):
        ...
        # imgA + masksA --> reconA transA
        # imgB + masksB --> reconB transB
        # that is to say, transA has masksA, so originalB_patches needs to have masksA

        batch_range, num_masked, rand_indicesA, masksA, pred_A2A, pred_A2B, patchesA = self.forward_ori(imgA)
        _, _, rand_indicesB, masksB, pred_B2A, pred_B2B, patchesB = self.forward_ori(imgB)

        masked_A2A = patchesA[batch_range, masksA]
        masked_A2B = patchesA[batch_range, masksB]
        masked_B2A = patchesB[batch_range, masksA]
        masked_B2B = patchesB[batch_range, masksB]

        # calculate reconstruction loss
        recon_loss_A = F.mse_loss(pred_A2A, masked_A2A)
        recon_loss_B = F.mse_loss(pred_B2B, masked_B2B)

        # calculate translation loss
        trans_loss_A = F.mse_loss(pred_A2B, masked_B2A)
        trans_loss_B = F.mse_loss(pred_B2A, masked_A2B)

        unshuffle = {}
        imgA2A = self._reconstrcuct_img_show(imgA.shape, rand_indicesA, num_masked, pred_A2A, patchesA,
                                             p_height=self.encoder.patch_height, p_width=self.encoder.patch_width)
        imgA2B = self._reconstrcuct_img_show(imgA.shape, rand_indicesA, num_masked, pred_A2B, patchesB,
                                             p_height=self.encoder.patch_height, p_width=self.encoder.patch_width)
        imgB2A = self._reconstrcuct_img_show(imgA.shape, rand_indicesB, num_masked, pred_B2A, patchesA,
                                             p_height=self.encoder.patch_height, p_width=self.encoder.patch_width)
        imgB2B = self._reconstrcuct_img_show(imgA.shape, rand_indicesB, num_masked, pred_B2B, patchesB,
                                             p_height=self.encoder.patch_height, p_width=self.encoder.patch_width)

        unshuffle["imgA2A"] = imgA2A
        unshuffle["imgA2B"] = imgA2B
        unshuffle["imgB2A"] = imgB2A
        unshuffle["imgB2B"] = imgB2B
        return recon_loss_A, recon_loss_B, trans_loss_A, trans_loss_B, unshuffle

    def forward_ori(self, img):
        ...
        tokens = self.patch_to_emb(patches)  # b 8*8 1024
        tokens = tokens + self.encoder.pos_embedding[:, 1:(num_patches + 1)]  # without the cls token
        tokens = tokens[batch_range, unmasked_indices]  # just 25% tokens
        ...
        add_styleA = self.A_style
        add_styleB = self.B_style
        decoder_tokensA = decoder_tokens + add_styleA[:, :]
        decoder_tokensB = decoder_tokens + add_styleB[:, :]
        decoder_tokensA = self.decoder(decoder_tokensA)
        decoder_tokensB = self.decoder(decoder_tokensB)

        # splice out the mask tokens and project to pixel values
        mask_tokensA = decoder_tokensA[:, -num_masked:]  # the first fews are removed
        mask_tokensB = decoder_tokensB[:, -num_masked:]
        pred_valuesA = self.to_pixels(mask_tokensA)
        pred_valuesB = self.to_pixels(mask_tokensB)

        return batch_range, num_masked, rand_indices, masked_indices, pred_valuesA, pred_valuesB, patches

    def forward_trans(self, imgs):
        ...
